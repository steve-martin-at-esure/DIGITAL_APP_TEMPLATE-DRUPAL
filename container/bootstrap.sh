...
for dn in `cat /var/www/html/container/web-writeable.txt`; do
...
/var/www/html/container/wait-for-db.sh
...
if [[ -n "$DRUPAL_INSTALL" && ! `drush cget system.site uuid` ]]; then
  ...
  CONFIG_DIR=/var/www/html/app/sync
...
if [[ $ENVIRONMENT == 'DEV' ]]; then
  cp /var/www/html/container/xdebug.ini /usr/local/etc/php/conf.d/xdebug.ini
fi
